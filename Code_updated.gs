...
function importarConferencia() {
  try {
    ...
    try {
      ...
      var FORBIDDEN_TERMS = ["fulfillment", "full", "cancelado"];
      ...
      // Same month detection as Discord
      ...
      function normConf_(s) {
        return String(s || "").trim().toUpperCase()
          .normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/\s+/g, " ");
      }
      function containsForbiddenConf_(text) {
        var t = normConf_(text).toLowerCase();
        return FORBIDDEN_TERMS.some(function(term) {
          var re = new RegExp("\\b" + term + "\\b");
          return re.test(t);
        });
      }
      var sheets = src.getSheets().filter(function(s) {
        var n = normConf_(s.getName());
        return months.some(function(m) { return n.indexOf(normConf_(m)) !== -1; });
      });

      // Load existing keys from Conferencia to avoid duplicates (key = pedido|mkt|produto|cliente|qtd)
      var existingKeys = {};
      var lastConfRow = conf.getLastRow();
      if (lastConfRow > 1) {
        var existData = conf.getRange(2, 1, lastConfRow - 1, 5).getValues();
        existData.forEach(function(r) {
          var k = [r[0],r[1],r[3],r[2],r[4]].join("|");
          existingKeys[k] = true;
        });
      }
      ...
      sheets.forEach(function(sheet) {
        ...
        // Read A..G (7 columns) — same as Discord
        var values = sheet.getRange(DATA_START_ROW, 1, numRows, 7).getValues();
        var bgG    = sheet.getRange(DATA_START_ROW, 7, numRows, 1).getBackgrounds();

        for (var i = 0; i < values.length; i++) {
          var row = values[i];
          var pedido    = String(row[1] || "").trim(); // col B
          var mkt       = String(row[2] || "").trim(); // col C
          var cliente   = String(row[3] || "").trim(); // col D
          var qtd       = row[4];                      // col E
          var produto   = String(row[5] || "").trim(); // col F
          var statusG   = normConf_(row[6]).toLowerCase(); // col G text
          var bgColor   = String(bgG[i][0] || "").toLowerCase(); // col G background

          if (!pedido) continue;
          if (statusG === "ok") continue;       // same as Discord
          if (bgColor === "#00ff00") continue;  // "verde" = já OK, same as Discord
          var rowText = row.join(" ");
          if (containsForbiddenConf_(rowText)) continue; // fulfillment, full, cancelado

          var key = [pedido, mkt, produto, cliente, qtd].join("|");
          if (existingKeys[key]) continue;
          existingKeys[key] = true;

          if (!pedidoCounters[pedido]) pedidoCounters[pedido] = 0;
          pedidoCounters[pedido]++;
          var itemKey = pedido + "#" + String(pedidoCounters[pedido]).padStart(2, "0");

          newRows.push([
            pedido, mkt, cliente, produto, qtd,
            "", "", "",   // Status, ConferidoPor, ConferidoEm
            importId,     // ID (I)
            itemKey,      // ItemKey (J)
            "PENDENTE",   // StatusPedido (K)
            "SIM"         // Confirmado (L)
          ]);
        }
      });
      ...
      if (newRows.length > 0) {
        conf.getRange(conf.getLastRow() + 1, 1, newRows.length, newRows[0].length).setValues(newRows);
      }

      // Regenerate Lista sheet
      gerarListaConferencia_();

      return { ok: true, importados: newRows.length, importId: importId };
    } finally {
      lock.releaseLock();
    }
  } catch(e) {
...
function gerarListaConferencia_() {
  try {
    ...
    if (!lista) lista = ss.insertSheet("Lista");
    lista.clearContents();
    ...
    data.forEach(function(row) {
      ...
      var status = String(row[5] || "").trim().toUpperCase(); // F
      var stPedido = String(row[10] || "").trim().toUpperCase(); // K
      if (!pedido || stPedido !== "PENDENTE") return;
      if (!pedidos[pedido]) { pedidos[pedido] = { mkt: row[1], cliente: row[2], items: [] }; order.push(pedido); }
      pedidos[pedido].items.push({ produto: row[3], qtd: row[4], status: status });
    });

    var listaRows = [["Pedido","Marketplace","Cliente","Produto","Qtd","Status"]];
    order.forEach(function(ped) {
      var p = pedidos[ped];
      p.items.forEach(function(it) {
        listaRows.push([ped, p.mkt, p.cliente, it.produto, it.qtd, it.status]);
      });
    });
    lista.getRange(1, 1, listaRows.length, 6).setValues(listaRows);
    lista.getRange(1, 1, 1, 6).setFontWeight("bold");
  } catch(e) {
    console.log("gerarListaConferencia_ erro: " + e);
  }
}
...
function buscarPedidosConferencia(q) {
  try {
    ...
    data.forEach(function(row, idx) {
      ...
      if (!pedido || stPedido !== "PENDENTE") return;
      if (pedido.toLowerCase().indexOf(q) === -1) return;
