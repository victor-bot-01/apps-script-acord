...
// ── Helpers internos do importarConferencia (portados do Discord) ─────────────
  ...
  try {
    ...
    try {
      ...
      var sheets = src.getSheets().filter(function(s) {
        var n = _icNormBusca_(s.getName());
        return months.some(function(m) { return n.indexOf(_icNormBusca_(m)) !== -1; });
      });
