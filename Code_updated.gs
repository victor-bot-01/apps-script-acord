  ...
  try {
    ...
    try {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        months.push(d.getMonth() + 1 < 10 ? "0" + (d.getMonth() + 1) + "/" + d.getFullYear() : (d.getMonth() + 1) + "/" + d.getFullYear());
      }
      var sheets = src.getSheets().filter(function(s) {
        return months.some(function(m) { return s.getName().indexOf(m) !== -1; });
      });

      // Get existing ItemKeys to avoid duplicates
      var existingKeys = {};
      var lastRow = conf.getLastRow();
      if (lastRow > 1) {
        var keyCol = conf.getRange(2, 10, lastRow - 1, 1).getValues();
        keyCol.forEach(function(r) { if (r[0]) existingKeys[r[0]] = true; });
      }
      ...
      sheets.forEach(function(sheet) {
        ...
        var iP = hdr.indexOf("pedido"); if (iP < 0) iP = 0;
        var iM = hdr.indexOf("marketplace"); if (iM < 0) iM = 1;
        var iC = hdr.indexOf("cliente"); if (iC < 0) iC = 2;
        var iPr = hdr.indexOf("produto"); if (iPr < 0) iPr = 3;
        var iQ = hdr.indexOf("qtd"); if (iQ < 0) { iQ = hdr.indexOf("quantidade"); } if (iQ < 0) iQ = 4;
        var iSt = hdr.indexOf("statuspedido"); if (iSt < 0) { iSt = hdr.indexOf("status pedido"); }

        for (var r = 1; r < data.length; r++) {
          var row = data[r];
          var pedido = String(row[iP] || "").trim();
          if (!pedido) continue;
          var stPedido = iSt >= 0 ? String(row[iSt] || "").trim().toUpperCase() : "";
          // Only import PENDENTE or empty status
          if (stPedido && stPedido !== "PENDENTE") continue;
          ...
          if (existingKeys[itemKey]) continue;
