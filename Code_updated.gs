...
function getPedidos(source) {
  try {
    ...
    // Não Marcado: lê da Conferencia (col F vazia, col K = PENDENTE)
    const todosNvItems = getItensNaoMarcadosConferencia(null).map(function(it) {
      return { produtoPendentes: it.produto, quantidade: Number(it.qtd) || 1, sheet: "Conferencia" };
    });
...
function enviarInformacoesNaoMarcado(selections) {
  try {
    ...
    for (const sel of selections) {
      ...
      // Usa todos os pedidos da Conferencia; source="" quando não existe no Dashboard
      const orders = [...orderIdSet]
        .map(id => ({ orderId: id, source: idSource.get(id) || "" }))
        .sort((a, b) => {
          const ai = PRIORITY.indexOf(a.source); const bi = PRIORITY.indexOf(b.source);
