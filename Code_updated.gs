...
const CONF_IMPORT_CONFIG = {
  ...
  DEST_COL_ITEMKEY: 10, DEST_COL_STATUS_PEDIDO: 11, DEST_COL_CONFIRMADO: 12,
};
...
function conf_preencherItemKey_() {
  ...
  const reImp = /^IMP-\d{8}-(\d+)$/;
  let counter = 0;
  for (let i=0; i<numRows; i++) {
    const m = String(colJ[i][0]||"").trim().match(reImp);
    if (m) counter = Math.max(counter, parseInt(m[1], 10));
  }
  const outJ = colJ.map(r=>[r[0]]);
  for (let i=0; i<numRows; i++) {
    if (String(outJ[i][0]||"").trim()) continue;
    const hasAny = dataAE[i].some(v=>String(v||"").trim()!=="");
    if (!hasAny || !conf_normalize_(dataAE[i][0])) { outJ[i][0]=""; continue; }
    counter++;
    outJ[i][0] = "IMP-" + dateStr + "-" + String(counter).padStart(3,"0");
  }
